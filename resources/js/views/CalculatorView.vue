    ...
    <section class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <article class="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-6">
            <div class="mb-4 space-y-2">
                ...
                <div data-test="expression-display" class="min-h-14 rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-right text-2xl font-semibold break-all">
                    {{ expression || '0' }}
                </div>
        ...
        <aside class="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-6">
            ...
            <ul v-if="history.length" data-test="history-list" class="space-y-2">
                <li
...
const submitCalculation = async () => {
    ...
    try {
        ...
        history.value = [item, ...history.value.filter((historyItem) => historyItem.id !== item.id)];

        feedback.value = `Result: ${item.result}`;
        expression.value = '';
    } catch (error) {
        const message = error?.response?.data?.errors?.expression?.[0]
